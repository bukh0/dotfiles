...
DELETIONS_DIR_NAME = "Deletions"
DELETIONS_DIR = os.path.join(WALLPAPER_DIR, DELETIONS_DIR_NAME)
...
def extract_palette(image_path, num_colors=8):
    try:
        with Image.open(image_path) as img:
            img = img.convert("RGB").resize((160, 160)).convert("P", palette=Image.ADAPTIVE, colors=num_colors)
...
def unique_deletions_path(filename):
    dest = os.path.join(DELETIONS_DIR, filename)
    ...
    n = 1
    while True:
        candidate = os.path.join(DELETIONS_DIR, f"{base} ({n}){ext}")
        if not os.path.exists(candidate):
            return candidate
        n += 1


def sync_deletions(current_files):
    print("\nPhase 2: Syncing deletions to theme subdirectories...")
    os.makedirs(DELETIONS_DIR, exist_ok=True)

    current_set = set(current_files)
    moved_count = 0

    for theme in THEMES:
        theme_dir = os.path.join(WALLPAPER_DIR, theme)
        if not os.path.isdir(theme_dir):
            continue

        for f in os.listdir(theme_dir):
            src_path = os.path.join(theme_dir, f)
            if not os.path.isfile(src_path):
                continue

            if f not in current_set:
                dest_path = unique_deletions_path(f)
                shutil.move(src_path, dest_path)
                print(f"  Moved: [{theme}] {f} -> {DELETIONS_DIR_NAME}/{os.path.basename(dest_path)}")
                moved_count += 1

    if moved_count == 0:
        print("  No stale copies found.")
    else:
        print(f"  Moved {moved_count} stale file(s) into {DELETIONS_DIR_NAME}/")
...
def main():
    ...
    sync_deletions(files)

    print("\nPhase 3: Checking and upscaling low-resolution master images...")
    upscaled_log = []
    for f in files:
    ...
    print(f"\nPhase 4: Categorizing {len(files)} wallpapers with CIELAB perception...")
    copied_count = 0
