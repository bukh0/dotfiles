...
hl.bind(
	mod .. " + CTRL + S",
	hl.dsp.exec_cmd('grim -g "$(slurp)" ~/Pictures/Screenshots/screenshot-$(date +%Y%m%d-%H%M%S).png')
)
hl.bind(
	mod .. " + CTRL + Print",
	hl.dsp.exec_cmd(
		'FILE="$HOME/Pictures/Screenshots/screenshot-$(date +%Y%m%d-%H%M%S).png"; grim -g "$(slurp)" "$FILE" && notify-send -a "Screenshot" -i "$FILE" -h string:image-path:"$FILE" "Screenshot Saved" "Region captured"'
	)
)
...
hl.bind(
	mod .. " + Print",
	hl.dsp.exec_cmd(
		'FILE="$HOME/Pictures/Screenshots/screenshot-$(date +%Y%m%d-%H%M%S).png"; grim "$FILE" && notify-send -a "Screenshot" -i "$FILE" -h string:image-path:"$FILE" "Screenshot Saved" "Fullscreen captured"'
	)
)
...
hl.bind(
	mod .. " + SHIFT + N",
	hl.dsp.exec_cmd([[bash -c "pkill -x wlsunset || wlsunset -l -25.74 -L 28.18 -t 4500 -T 6500 &"]])
)
...
hl.bind(mod .. " + ALT + 0", hl.dsp.exec_cmd("~/.scripts/toggle_quickshell.sh"))
